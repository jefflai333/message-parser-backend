...
def before_request():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS messages (id SERIAL, date TIMESTAMP NOT NULL, word_count INTEGER NOT NULL, char_count INTEGER NOT NULL, conversation_id INTEGER NOT NULL, sender_participant_id INTEGER NOT NULL, PRIMARY KEY (id), FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE CASCADE, FOREIGN KEY (sender_participant_id) REFERENCES participants(id) ON DELETE CASCADE);")
    cur.execute(
